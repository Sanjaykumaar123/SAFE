...
from app.services.fleet.demo_store import (
    DEMO_CITY_ID,
    DEMO_OPERATOR_CODE,
    DEMO_PASSWORD,
    DEMO_USER_ID,
...
from app.schemas.fleet import FleetOperatorOut, VehicleOut
...
def _get_demo_auth_response() -> FleetAuthResponse:
    access_token = create_access_token(str(DEMO_USER_ID))
    refresh_token, _ = create_refresh_token(str(DEMO_USER_ID))
...
async def fleet_login(payload: FleetLoginRequest, db: AsyncSession = Depends(get_db)) -> FleetAuthResponse:
    code = payload.operator_code.strip().upper()
    if settings.DEMO_MODE and code == DEMO_OPERATOR_CODE:
        if payload.password == DEMO_PASSWORD:
            return _get_demo_auth_response()
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, INVALID_CREDENTIALS)

    try:
        profile_result = await db.execute(select(FleetOperatorProfile).where(FleetOperatorProfile.operator_code == code))
        ...
        if code == DEMO_OPERATOR_CODE and payload.password == DEMO_PASSWORD:
            return _get_demo_auth_response()
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, INVALID_CREDENTIALS)

...
async def create_observation(
    ...
    try:
        ...
        obs_id = uuid.uuid4()
        now_dt = payload.observed_at or datetime.now(timezone.utc)
        return ObservationOut(
            id=obs_id,
            hazard_id=uuid.uuid4(),
            vehicle_id=str(ctx.vehicle.id if ctx.vehicle else uuid.uuid4()),
            operator_id=ctx.profile.operator_code,
            observed_at=now_dt,
            confidence=payload.confidence,
            latitude=payload.latitude,
            ...
            vehicle_ref_id=ctx.vehicle.id if ctx.vehicle else uuid.uuid4(),
            operator_ref_id=ctx.profile.id,
            client_observation_id=payload.client_observation_id,
            hazard_type=payload.hazard_type.value,
            model_name=payload.model_name,

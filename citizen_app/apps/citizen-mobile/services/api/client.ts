/**
 * The one centralized API client (section 35). No screen or feature module
 * calls `fetch`/`axios` directly — everything goes through `apiClient` or a
 * `services/*` wrapper around it.
 *
 * Responsibilities: base URL, auth header injection, request timeout,
 * snake_case<->camelCase translation, 401 -> refresh-once-> retry, and
 * normalizing every failure into the `ApiError` shape so screens never
 * branch on raw axios internals.
 */
...
export function toApiError(error: unknown): ApiError {
  if (axios.isAxiosError(error)) {
    if (!error.response) {
      return { status: null, message: 'You appear to be offline. Showing recently cached road data.', isNetworkError: true };
    }
...
/** Safe-to-retry GET wrapper — retries transient network failures once
 * (never used for POST/PATCH/DELETE, which must not be silently retried). */

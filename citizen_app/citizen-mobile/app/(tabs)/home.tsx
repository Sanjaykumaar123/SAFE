  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={['top']}>
      ...
      {home.isPending && !home.data ? (
        ...
        >
          <HazardMapPreview center={coords ?? DEFAULT_MAP_CENTER} hazards={home.data?.mapMarkers ?? DEMO_HOME.mapMarkers} onSelectHazard={openHazard} />

          <View style={styles.section}>
            <Text style={styles.sectionTitle}>ROAD STATUS NEAR YOU</Text>
            <View style={styles.statsRow}>
              <StatCard label="Hazards" value={home.data?.stats?.nearbyCount ?? DEMO_HOME.stats.nearbyCount} />
              <StatCard label="Critical" value={home.data?.stats?.criticalCount ?? DEMO_HOME.stats.criticalCount} tone="critical" />
              <StatCard label="Warnings" value={home.data?.stats?.warningCount ?? DEMO_HOME.stats.warningCount} tone="warning" />
            </View>
          </View>

          <View style={styles.section}>
            <Text style={styles.sectionTitle}>NEARBY HAZARDS</Text>
            {(home.data?.nearbyHazards ?? DEMO_HOME.nearbyHazards).length ? (
              <View style={styles.hazardList}>
                {(home.data?.nearbyHazards ?? DEMO_HOME.nearbyHazards).map((hazard: Hazard, index: number) => (

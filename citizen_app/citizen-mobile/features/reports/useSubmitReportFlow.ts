...
export function useSubmitReportFlow() {
  ...
  async function submit() {
    ...
    try {
      ...
      let report: any;
      try {
        report = await createReport.mutateAsync({
        ...
        console.warn('Backend report submission failed, storing locally and queuing sync:', err);
        report = await reportsApi.createDemoFallback({
          hazardType: store.hazardType,
          severity: store.severity,
          description: store.description || undefined,
          latitude: store.location.latitude,

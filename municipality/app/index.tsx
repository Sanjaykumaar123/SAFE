...
/**
 * SPLASH -> AUTH CHECK -> LOGIN (if unauthenticated) -> (TABS) (section 11).
 * Waits for auth bootstrap AND a minimum splash duration so the brand
 * moment always shows. `(tabs)/_layout.tsx` is what actually fetches the
 * municipality profile/authorized cities before rendering Dashboard —
 * this screen only decides authenticated vs. not.
 */
  ...
  if (authStatus === 'checking' || !minTimeElapsed) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.appBadge}>MUNICIPALITY</Text>
      </View>
    );
  }

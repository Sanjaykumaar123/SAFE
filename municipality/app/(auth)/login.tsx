...
const loginSchema = z.object({
  municipalityCode: z.string().min(2, 'Enter your municipality ID (e.g. MUN-CHN)'),
...
export default function LoginScreen() {
  ...
  const {
    ...
    defaultValues: { municipalityCode: 'MUN-CHN', email: 'officer@chennai.gov.in', password: 'SafePath@123' },
  });
  ...
  const fillDemo = () => {
    setValue('municipalityCode', 'MUN-CHN');
    setValue('email', 'officer@chennai.gov.in');
  ...
  const onSubmit = async (values: LoginForm) => {
    ...
    try {
      ...
      setSubmitError(error instanceof Error ? error.message : 'Login failed. Check your municipality ID, email, and password.');
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        ...
        <Text style={styles.subtitle}>Municipality Sign-In</Text>
        ...
        <View style={styles.form}>
          ...
          <Text style={styles.demoHint} onPress={fillDemo}>Demo: MUN-CHN · officer@chennai.gov.in · SafePath@123 (Tap to fill)</Text>
        </View>
      </ScrollView>

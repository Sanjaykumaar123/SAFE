/**
 * The one centralized API client. No screen or feature module calls
 ...
 * direct database connection — see README.md "Architecture"). Token
 * refresh/logout reuse the backend's role-agnostic `/auth/refresh` and
 * `/auth/logout` — the same endpoints the Citizen app calls — since a
 * refresh token is just a revocable session row keyed by user id,
 * independent of which app issued it.
 */

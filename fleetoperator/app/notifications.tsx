...
/** §57 — TRIP_VALIDATED / PAYMENT_APPROVED / device warnings, etc. Same
 * polling-list pattern every SafePath app uses (no push registration —
 * see DEFERRED.md). */
    ...
    <View style={[styles.flex, { paddingTop: insets.top + spacing.sm }]}>
      ...
      {data ? (
        <FlatList
          ...
          renderItem={({ item }) => <NotificationRow notification={item} onPress={() => !item.isRead && markRead.mutate(item.id)} />}
